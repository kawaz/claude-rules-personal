...
公開できない参照知識 (kawaz の好み・環境の癖・私的な事情) は、公開候補のこのリポではなく private リポ `kawaz/privacy-personal` に同じ構造で置く (業務面でしか効かないものは各 overlay の `for-me/`)。
読み書きの手順は `rules-personal:knowledge` skill の「private の参照知識」節。
