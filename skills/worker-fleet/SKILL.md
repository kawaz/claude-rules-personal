...
- **codex 系の「不安定」を理由に避けるのは、実測で裏付けが無い限りバイアスとして疑う**
  (2026-08-11 アンケートで、過去の不安定実績を無批判に踏襲して codex を避けていた
  セッションを発見。同時期の別セッションは codex-sol を問題なく多用していた)
- **同じ codex agent で effort だけ一時的に上げたい** (例: 難問だけ luna を `xhigh` で)
  というニーズは 2026-08-11 時点で実証されていない。常設 agent は増やさない。
  frontmatter が唯一の制御点である以上 (禁則節参照)、委譲プロンプト内に effort を
  書いても実際の API パラメータには反映されない — 本当に必要になったら
  `codex-luna-worker-xhigh` のような新規 agent 定義を作るしかない
