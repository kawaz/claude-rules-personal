...
セクション構成は `pre-clear` skill §2 の 10 項と同一 (Recovery Notes 内の §継続
作業指示 小節も同型で書く)。compact では以下に厚く倒す:
...
- **要約器が落とす精密情報の保険**:
  ...
  - **§継続作業指示** — 要約が「作業中の話題」として圧縮すると誤読の元、原文の
    「次にやる作業」として明示保持。**出どころラベル (`[kawaz 指示]` / `[AI 自律]`、
    書式の正本は `pre-clear` skill) 込みで書く** — 要約はラベルを真っ先に落とすため
...
**§継続作業指示 (§10 Recovery Notes 内) があれば、要約前に取り組んでいた作業が
消えているかを確認し、消えているなら出どころラベルで分岐して着手する** —
`[kawaz 指示]` は即実行、`[AI 自律]` (およびラベル無し) は関連 DR / issue /
QUESTIONS.md の裁定と矛盾しないことを確認してから実行する (`pre-clear` skill の
ロード側プロトコルと同じ判断)。走行中作業とマージする場合は継続作業指示を優先度
リストの上位に置く。「要約が話題として書いた」と「継続作業指示が明示している」は
別軸で、後者が上位判定材料。
