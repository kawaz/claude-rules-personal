...
- **現コンテキストにしか無い永続知識** — 状態ファイルに書く前に §1.5 の永続化ファーストで
  リポ側へ落とす (state は消えても良い運用状態だけを持つ場所)
...
2. **鮮度チェック**: cwd 照合 OK でも、`state:` の日付と本文 §2 Current Phase の
   head (コミット hash) を実機 (`jj log -r main` / `git log`) と照合する。**main が
   state 記録時より進んでいたら、そのハンドオフの §継続作業指示は失効扱い** —
   別セッションが消化した可能性が高い (実例: v0.21.1 時点の指示が残った state を
   v0.26.0 のセッションがロードしかけた 2026-08-27)。失効時は指示を実行せず、
   実機確認の結果と次アクション候補の提示に降格する (state の他セクションは
   背景知識として読んで良い)
3. **cwd 照合 OK かつ `loaded_by:` が空なら**: `state:` の実体を Read し、直後に
   `loaded_by:` へ 1 行追記 (`  - <ISO時刻> <自分の session-id>`)。以後このハンドオフは
   自分のもの
4. **他セッションの記録が既にあるなら**: このハンドオフは**その系列に消費済み**なので
...
落とした後、state からは**参照で指す** (本文を二重化しない)。この順序を守ると、
継続指示の多くが「<doc> を読んで実行」の 1 行になり、粒度問題も同時に消える。
...
6. Worker & Peer Topology — ★clear 固有: worker ハンドルは失効するので「再 spawn
                          手順」として書く (agent 定義名・使い分けの実績・委譲プロンプトの型)。
                          **連携中の別セッション (ccmsg peer) は「sid + cwd + 交わした約束」
                          だけを列挙する** — room 番号や aN は後継が room に参加して
                          いないので使えない。後継は相手に「{cwd} の新セッションです、
                          前任の約束を引き継ぎます」と名乗って再接続する (相手側も
                          同じ規約なら cwd で照合できる)
7. Skills Invoked       — 使った skill と、その中で得た運用知見 (罠・定石)
8. Editing Files        — 主要ファイルと設計正本の所在。**意図的にリポ外へ置いた
                          生成物** (例: 業務 cwd を含むレポートを `~/.cache` へ) は
                          パスと理由を列挙する (後継が「消して良いか / 参照すべきか」を
                          判断できるように)
...
10. Recovery Notes      — ★clear 固有: 一次資料を読む順序 (立ち上げの最短経路)、
                          ...
                          **§継続作業指示を必ず明示的小節として立てる** — 「次
                          ...
                          付けて書く (ロード側プロトコル §1 Step 5 の分岐がここで
                          判定される)
```
