...
(kawaz r119 2026-08-15 で実機確認済みの経路)。**`/clear` は必ず引数付きで打つ**:
...
- **引数はクリア後の最初のプロンプトとして投入される**。素の `/clear` だと後継セッションは
  SessionStart hook の指示 (role skill のロード等) を済ませただけで指示待ちに入り、
  state ファイルを読まない [実機確認: 2026-08-20]
- 上の 1 文が**継続作業指示の有無を両方カバー**するので、pre-clear 側で打ち分けない
  (有無で分岐すると「無い場合 = 素の `/clear`」になり、後継が state を読まず停止する)
