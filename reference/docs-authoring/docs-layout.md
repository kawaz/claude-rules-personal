...
- INDEX.md の表は `| DR | 状態 | 説明 |`。状態は**絵文字とラベルだけ** (日付・Phase・裁定の内訳は各 DR 本文の `Status:` 行に書き、INDEX には持たない): `✅ 実装済` (Decision の全部に実装エビデンスがある) / `🟡 部分実装` / `⬜ 未実装` (設計のみ) / `🚧 進行中` / `N/A` (命名・思想・プロセス等、実装対象でない) / `❌ 撤退`。状態を変える変更は DR 本文の `Status:` 行と INDEX を同じ commit で更新する (実例: kawaz/hyoui)
