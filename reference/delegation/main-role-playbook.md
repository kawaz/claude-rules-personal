...
- 統括は自分のターンを塞ぐ待ち (`until` / `sleep` のループ、数分かかる test や push) を前景の Bash に置かない。長いものは `run_in_background` に出し、kawaz への返答を先にする。前景で待つと、その間 kawaz の発言に返せず会話が止まる (kawaz 2026-09-17)。負荷や外部条件を待つループ自体も置かない (落ちるなら落ちる理由を直す)
...
- worker 起草 DR は別 worker で検査する (fable-high or reviewer-sol-high、意味論の穴 vs 機械確認寄りで選ぶ)。検査結果は finding 側の修正 + Q 起票 + kawaz 裁定へ回す
