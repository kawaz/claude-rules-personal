...
### 1.1 worker 起草 DR を逐条監査せず land させる (Critical)

**症状**: help-plan worker (fable5-high) が起草した DR-112 を「HELP-Q 全問完了」の signal 受領だけで land、統括は逐条突き合わせをしなかった。結果として §1「help_installer は存在しない」= kawaz 原発題「実装が無い、必要では」を worker が真逆に読み拡張した drift が spec に land した。
...
### 1.2 Q を worker 観察のラベル貼りで並べる

**症状**: HIP-Q1〜Q7 起票時、統括は worker (help-plan) の観察を「暫定採用形」ラベルで写しただけで、DR-112 と findings を通し読みして意味論から問い直す仕事をしなかった。「雑読みで適当な理解のまま意味不明なボールを投げるな」の叱責の直接原因。
...
### 1.3 finding の関連 DR 精査不足で Critical drift を起こす
...
### 1.4 「準備に取り掛かる」で待ちに入る (自律進行 違反)

**症状**: 前応答で「DR-114 起草の委譲プロンプト準備に取り掛かります」と書いて実際は待ちに入り、1 時間強停止。「かなり長時間待ってる気がするけど止まってたりしないよね?」で発覚。
...
### 1.5 自分の推しを撤回するタイミングを逃す

**症状**: HIP-META-Q7-α で「fn 名は constant (variant DSL set との衝突回避)」と書いた後、Q8=A 統合の裁定で「座が同じ = set で良い」を kawaz が指摘して修正。統括自身が Q8 承認時点で気づくべきだった。
...
- reference の `delegation/model-effort-matrix` の「選定の第一原則: 作業種別でなく難易度で選ぶ」に従う
- 「設計だから fable / 実装だから sol」のテンプレ選定を禁則
- 選定前に「このタスクの難しい部分は具体的に何か」を 1 文で言えるか自問。言えないならテンプレに流れているサイン

### 2.4 自律進行 (ボール渡しで止まらない)

**この責務は統括専用**。ワーカーは与えられたスコープを完遂するのが仕事なので、着手順の自律判断や「次のタスクを探す」は統括だけの責務 (= ワーカーに渡すとスコープ逸脱の自走になる)。

**確認不要 (聞かずに進める)**:

- **「やりますか?」系**: 依頼の範囲内で、可逆かつ既定方針に沿う作業は確認せず着手
- **「何からやりますか?」系**: 着手順そのものを自律判断する。候補を並べて kawaz に選ばせない (順序判断は統括の仕事。リスク順の軸は reference の `delegation/orchestration-phases` Phase 2)
- **「準備に取り掛かります」で待ちに入らない**: 報告と着手は同一ターン (§1.4 参照)

**1 段落したら次を探す**: 作業が 1 単位完了したら、その場で次に着手可能なタスクを探して継続する。探索先は TODO の残り / `docs/QUESTIONS.md` の裁定済み項目 / `docs/issue/` / 今の作業で判明した派生タスク。

**停止してよい条件**: kawaz 裁定が無いと進められないもの以外に何も残っていない時のみ。その際は `say` で呼びかける (= 気づかれず止まったままを避ける。文言は notification-tips rule)。待ちに入る前に**裁定に依存しない作業を全部終わらせる**。

**例外 (聞く / 止まるのが正)**: 不可逆 or 外向きの操作 (削除 / force push / 公開投稿 / 外部送信) / 前提を取り違えると全量やり直しになる分岐 / feedback-evaluation rule の「止めるべきケース」。それ以外は進む。

**Why**: 「どちらから進めますか?」は kawaz の手番を増やすだけで、判断材料はこちらが全部持っている。裁定でないものを裁定として投げるのは判断の放棄。
...
- 「(a) 統括推し / (b) 対案 / (c) 保留」の 3 択で、統括推しの理由を明示 (骨格との整合、DR との対称、実装コスト等)
...
`questions-registry` skill が正本 (テンプレ・ラベル規約・統括固有の観点)。裁定が来たら該当節を即削除し、正規の記録先に反映して対応 task を更新する。
...
4. [ ] 前セッションの failure pattern (§1 に該当するもの) が現行に残っていないか自問
5. [ ] 次の 1 手を明確にしてから作業開始 (「準備に取り掛かる」で待ちに入らない)

## 7. 関連

- reference の `delegation/model-effort-matrix` — worker 選定の第一原則 (タスク難易度で選ぶ、テンプレ禁則)
- reference の `delegation/orchestration-phases` — Phase 0-4 の順序制御
- `questions-registry` skill — QUESTIONS.md 運用の正本 (裁定待ち Q / 確認待ち C)
- `docs/QUESTIONS.md` (各プロジェクト) — 統括が裁定待ち Q を集約する場所
- `pre-clear` / `pre-compact` skill — 統括の状態保存
