...
System Extension (NetworkExtension の `NETransparentProxyProvider` 等) を含むアプリは、 通常の CLI/.app 署名に加えて **プロビジョニングプロファイル + 専用 entitlements** が要る。詳細手順は当該プロダクトリポの該当 PoC doc を参照する。
...
参考プロダクト (業務系) の doc には実 Team ID・法人名・業務 Bundle ID が載っているが、**個人リポには実値を書かない** (「自分の Team ID」「`<extension-bundle-id>`」等に一般化)。具体値が要る時は当該プロダクトリポの PoC doc を直接見る。
