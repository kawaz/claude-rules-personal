...
規約なのは**考え方** — 何と何を並記するか、答えられる相手が居ないときどう答えるか、どの条件で「上げ直しが要る」と言えるか。以下の field 名は形を示すための例示で、命名は各ツールで判断する。時刻は ISO 8601 の文字列か秒単位の整数にし、単位を名前に埋める形 (`_ms` suffix 等) は採らない。
...
| `started_at` | 今の子が起きた時刻 (ISO 8601)。居なければ省略 |
