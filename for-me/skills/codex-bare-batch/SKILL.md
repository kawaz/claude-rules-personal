...
- `CLAUDE_CODE_MAX_CONTEXT_TOKENS`: 200k の壁は **Claude Code クライアント側の自己抑制**
  で、proxy にも upstream にも無い (250k 直投げが HTTP 200、2026-07-15 実測)。この env で
  解除でき、input 200k 超の成功を実測済み。GPT-5.6 の素の window は 1.05M。ただし
  272K 超入力は割増料金 (入力 2 倍・出力 1.5 倍) がリクエスト全体に掛かるので、
  常用は ~270K までに留める

- model は用途で選ぶ: `gpt-5.6-sol` (レビュー・監査・高難度) / `gpt-5.6-terra`
  (通常) / `gpt-5.6-luna` (軽量)
- 業務面 (emeradaco) では port を **8318** に変える (認証境界、
  [[cliproxyapi-codex-usage]])
...
- [[cliproxyapi-codex-usage]] — codex の経路・面分離・preset 一覧の正本
