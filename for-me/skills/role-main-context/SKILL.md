...
description: 統括メインの役割と失敗パターンの再発防止。load-role-main から invoke される。
---
...
### 1.4 自律進行 (ボール渡しで止まらない)

**この節は統括専用**。ワーカーは与えられたスコープを完遂するのが仕事なので、
着手順の自律判断や「次のタスクを探す」は統括だけの責務 (= ワーカーに渡すと
スコープ逸脱の自走になる)。だから常時ロード rule ではなく本 skill に置く。

**確認不要 (聞かずに進める)**:

- **「やりますか?」系**: 依頼の範囲内で、可逆かつ既定方針に沿う作業は確認せず着手
- **「何からやりますか?」系**: 着手順そのものを自律判断する。候補を並べて kawaz に
  選ばせない (順序判断は統括の仕事。リスク順の軸は `orchestrate` skill Phase 2)
- **「準備に取り掛かります」で待ちに入らない**: 報告と着手は同一ターン (§2.4 参照)

**1 段落したら次を探す**: 作業が 1 単位完了したら、その場で次に着手可能なタスクを
探して継続する。探索先は TODO の残り / `docs/QUESTIONS.md` の裁定済み項目 /
`docs/issue/` / 今の作業で判明した派生タスク。

**停止してよい条件**: kawaz 裁定が無いと進められないもの以外に何も残っていない時のみ。
その際は `say` で呼びかける (= 気づかれず止まったままを避ける。文言は
[[notification-tips]])。待ちに入る前に**裁定に依存しない作業を全部終わらせる**。

**例外 (聞く / 止まるのが正)**: 不可逆 or 外向きの操作 (削除 / force push / 公開投稿 /
外部送信) / 前提を取り違えると全量やり直しになる分岐 / [[feedback-evaluation]] の
「止めるべきケース」。それ以外は進む。

**Why**: 「どちらから進めますか?」は kawaz の手番を増やすだけで、判断材料はこちらが
全部持っている。裁定でないものを裁定として投げるのは判断の放棄。
...
### 2.4 「準備に取り掛かる」で待ちに入る (§1.4 自律進行 違反)
