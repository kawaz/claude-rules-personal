...
各リポのタスクランナーは **`justfile`** を canonical とする。kawaz/bump-semver の `justfile` が基準実装。新規リポは「近い構成のリポ」の justfile を参考に書く (テンプレを SKILL に embed しない — 実ファイルが常に正本、言語別テンプレも作らない)。
