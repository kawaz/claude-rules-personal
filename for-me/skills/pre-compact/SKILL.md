...
`pre-clear` skill と同一: `${XDG_CACHE_HOME:-$HOME/.cache}/claude-session-state/<project-slug>/`
に `<YYYYMMDD-HHMM>.md` + `latest.md` コピー。
...
自動注入は無いので、**compaction 直後の最初のターンで latest.md を Read し直す**。
(summary に本ファイルへの言及が残るとは限らない — 状態ファイル側が正、summary は補助)
