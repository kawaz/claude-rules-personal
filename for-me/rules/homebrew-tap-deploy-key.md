# Homebrew Tap Deploy Key パターン

kawaz/<project> から kawaz/homebrew-tap への自動 push を有効にするための deploy key 運用ルール。
...
作業前に **kawaz に 1 回だけ Yes/No を AskUserQuestion で取る**。承認後は手順を中断せず実行。
確認文例: 「kawaz/<project> 用の HOMEBREW_TAP_DEPLOY_KEY セットアップ (鍵生成 → secret 登録 → tap の deploy key 登録 → 失敗 run の rerun) を進めて良いですか？」
...
- GitHub Docs: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/managing-deploy-keys
- 参考実装: kawaz/{authsock-warden, stable-which, authsock-filter} の release.yml が同じパターン (`<project> release` タイトルで tap 側に並ぶ)
