...
このうち機械検査できる項目 (リンク方向 / 自己参照 / `.draft-` 配置 = fatal、5KB 超 =
warning) は `just lint-rules` に集約済み。**push の deps で自動実行される**ため通常は
push task がまとめて弾く。commit 前に単体で確認したければ `just lint-rules` を実行する。

「未来予告 / 過去 narrative」(`[[no-historical-noise]]`) だけは機械判定が難しいので
目視で確認する。
