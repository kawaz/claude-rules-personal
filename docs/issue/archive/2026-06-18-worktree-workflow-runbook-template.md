...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-28T22:13:27+09:00
discard_reason:
pending_reason:
close_reason: ["done:docs-structure skill に templates/runbooks/worktree-workflow.template.md を新設、SKILL.md にテンプレ表とworktreeからのpush gate節を追加 (commit d91e2bb7)","done:blocked-by だった bump-semver 側は DR-0038 で実装済み","done:gate predicate は is on-default-branch 反転 (is worktree は main workspace 誤検出、実機確認済み)","done:既存リポへの一斉 migration は 2026-06-22 issue の通りスコープ外、adopt 手順として個別導入手順を記載"]
blocked_by:
origin:
---
