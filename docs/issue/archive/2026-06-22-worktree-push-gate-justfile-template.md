...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-28T22:14:06+09:00
discard_reason:
pending_reason:
close_reason: ["done:docs-structure skill の templates/runbooks/worktree-workflow.template.md に adopt 手順 (check-on-default-branch / sync / promote の 3 recipe + push deps) として実装 (commit d91e2bb7)。受け入れ条件4点充足。justfile サンプルは DR-0038 Adoption pattern 不採用のため is on-default-branch 反転で採用。既存リポ群への一斉 migration は別スコープ"]
blocked_by:
origin: 自リポ TODO
---
