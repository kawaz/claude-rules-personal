...
- **参照知識 (`skills/knowledge/reference/<slug>.md`)** = **読むだけの知識**。実行資源を伴わない。個別 skill を増やさず `knowledge` skill の索引に登録し、必要時にそのファイルだけ Read させる
...
3. **リンク規約**: wikilink (`[[名前]]` 記法) は常時ロード同士でのみ使う。`for-all/rules/` から `for-me/rules/` への参照は禁止 (overlay 越境で dead link 化)。skill への参照は「`<name>` skill 参照」と名前で書く
...
[[no-historical-noise]] の「除外リストを書くな」と、tdd-and-test-design の「該当なしを明示する勇気」は文書種別で切り分ける:
