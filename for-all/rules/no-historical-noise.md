...
- **除外リスト・「効かないもの」「やらないこと」列挙**: ルール / runbook / spec / trigger 定義に「効かないトリガ」「該当しない」「除外」セクションを作らない。
  書くべきは「該当するもの」だけ。除外列挙は書き手の説明欲・自己満足で、読み手の利便性ではなく文書を汚す。
  「拾うべきもの」を網羅すればそれ以外は自動的に拾われない (= 包含側で書け)。
  例外: セキュリティ / 不可逆操作の禁止 (= `rm -rf /` 禁止のような失敗防止) は別 — これは明示禁止に正当な理由がある。
