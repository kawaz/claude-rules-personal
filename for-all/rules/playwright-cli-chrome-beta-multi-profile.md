...
`agent-browser-session-isolation` skill (for-me/skills/) の規約に揃える: `<タスク種別>-<issue/PR番号>-<担当>` 形式 (例: `survey-1234-kawazu`, `fix-1234-kawazu`)。
...
- `agent-browser-session-isolation` skill (for-me/skills/) — session 命名規約 (= skill 化済、agent-browser 利用時に発火)
