...
**`SSH_AUTH_SOCK` を export だけしても github.com への ssh では効かない**。
`~/.ssh/config` の `Host *` で `IdentityAgent` が指定されている場合、
そちらが環境変数より優先される (= 環境変数 `SSH_AUTH_SOCK` は無視される)。

代わりに `Match exec` で動的判定する設計になっている:
...
- 越境作業時は **`cd /path/to/env-Y/<repo>`** に入れば ssh-agent も自動切替
- `SSH_AUTH_SOCK=` を別途 export する必要なし (むしろ無意味)
- git remote が env-Y のアカウントを指していれば、cwd が別の場所でも切替が効く
  (例: worktree が別 directory にあるが remote が env-Y/* なら OK)

### jj/git の commit signing にもこの仕組みが効くか

**部分的に効く**。`Match exec` は ssh コマンド時に評価されるため、git/jj が
内部で ssh を呼ぶ push 等では cwd 判定が発動する。**ただし jj の
`signing.key` (commit に署名する公開鍵) は global config で固定値が
入っている場合があり、その鍵が現在の agent に居なければ署名失敗する**。

```
Internal error: Could not write object of type commit
Caused by: Signing error / SSH sign failed
1: No private key found for "/var/folders/.../jj-signing-key-..."
```

#### 対策

別アカウント側の鍵で署名するリポでは、**repo-local config で `signing.key` を override**:

```bash
cd /path/to/env-Y/<repo>
# 現 ssh-agent の鍵を採用 (Match exec で切替済み)
jj config set --repo signing.key "$(ssh-add -L | head -1 | awk '{print $1, $2}')"
```

これで以降の push 署名は agent が持つ鍵で行われる。
**default 環境 (個人面) のリポは override しない** (global config が正)。
