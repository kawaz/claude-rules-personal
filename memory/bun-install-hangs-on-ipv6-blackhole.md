...
perl -pi -e 's#, "http://127\.0\.0\.1:4873/[^"]*\.tgz", \{#, "", {#g' bun.lock   # tarball URL ごと既定表現 (空文字) に戻す (区切りは `#`。`s{}{}` は zsh で中括弧が壊れる)。prefix だけ消すと `@types/bun/-/bun-1.4.2.tgz` のような相対値が残り CI の bun install が落ちる
